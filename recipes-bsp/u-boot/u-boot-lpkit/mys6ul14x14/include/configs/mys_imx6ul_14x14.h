...
#define CONFIG_EXTRA_ENV_LDKIT \
	"hwversion=0\0" \
	"type=mmc\0" \
	"system=A\0" \
	"rootfsdev=/dev/mmcblk0p1\0"\
	"rwdev=/dev/mmcblk0p2\0"\
	"fixbootargs=setenv bootargs console=${console},${baudrate} System=${system} ROOTIMG=${system}/rootfs.squashfs ROOTFSDEV=${rootfsdev} RWDEV=${rwdev} quiet\0"\
	\
	"load_dtb=ext4load ${type} ${mmcdev}:${mmcpart} ${fdt_addr} ${system}/mys6ul14x14-${hwversion}.dtb\0" \
	"load_kernel=ext4load ${type} ${mmcdev}:${mmcpart} ${loadaddr} ${system}/zImage\0" \
	"boot_kernel=run load_dtb; run load_kernel; run fixbootargs; bootz ${loadaddr} - ${fdt_addr}\0" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	CONFIG_MFG_ENV_SETTINGS \
	CONFIG_EXTRA_ENV_LDKIT \
	"fdt_addr=0x83000000\0" \
	"fdt_high=0xffffffff\0"	  \
	"console=ttymxc0\0" \
	"ethaddr=c0:c1:c0:13:0b:b8\0"\
	...
	"ip=dhcp nfsroot=${serverip}:${nfsroot},v3,tcp\0" \
		...
		"findfdt="\
			"if test $fdt_file = undefined; then " \
				...
				"if test $board_name = MYS6UL && test $board_rev = 14X14; then " \
					"setenv fdt_file mys-imx6ul-14x14-evk.dtb; fi; " \
				"if test $fdt_file = undefined; then " \
...
/* Optional use sd-card as boot dev*/
...
#define CONFIG_ENV_OFFSET		(504 * SZ_1K)
